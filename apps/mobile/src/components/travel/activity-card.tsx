...
const activityCardVariants = cva(
  'bg-card group relative rounded-lg border p-4',
  {
    variants: {
      preset: {
        default: 'border-border',
        travel: 'border-blue-200 bg-gradient-to-br from-white to-blue-50',
        timeline: 'border-border border-l-4 border-l-blue-500 pl-6',
...
export const ActivityCard: React.FC<ActivityCardProps> = ({
  ...
  const descriptionText = descriptionTx
    ? t(descriptionTx, descriptionTxOptions)
    : description;
  ...
  const categoryText = categoryLabelTx
    ? t(categoryLabelTx, categoryLabelTxOptions)
    : categoryLabel;
      ...
      <View className="flex-1 space-y-2">
        ...
        <View className="flex-row items-start justify-between">
          <View className="flex-1 flex-row items-center space-x-2">
            ...
            <Text
              className={`text-foreground flex-1 font-semibold ${
                size === 'small'
                  ? 'text-sm'
                  : size === 'large'
                    ? 'text-lg'
          ...
          <View className="flex-row items-center space-x-2">
            {showCategory && categoryText && (
              <View className="rounded-full bg-blue-100 px-2 py-1">
                <Text className="text-xs text-blue-800">{categoryText}</Text>
              </View>
            )}
